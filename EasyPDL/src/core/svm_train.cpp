...
namespace oir {
	...
	void Svm::divide(const char* images_folder, float percentage /* = 0.7 */) {
		...
		for (size_t i = 0; i < files.size(); ++i) {
			...
			assert(!image.empty());
			if (i < split_index) {
				sprintf_s(save_to, "%s/train/%s", images_folder, file_name.c_str());
			}
			else {
				sprintf_s(save_to, "%s/test/%s", images_folder, file_name.c_str());
			}
	...
	void Svm::get_train() {
		...
		for (auto f : files) {
			...
			auto features = oir::histeq(image);
